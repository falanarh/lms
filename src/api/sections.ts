...
export const getSections = async (): Promise<Section[]> => {
  const response = await axios.get<Section[]>(
    `${BASE_URL}/sections?idGroup=7d4a43cf-684a-4eac-98d1-05c800621492&orderBy[0][sequence]=asc`,
    {
...
export const getSectionsByGroupId = async (
  groupId: string,
): Promise<Section[]> => {
  const response = await axios.get(
    `${BASE_URL}/group-courses/${groupId}/section`,
  );
  return response.data.data;
};

export const createSection = async (
  newSection: Omit<Section, "id" | "group" | "createdAt" | "updatedAt">,
): Promise<Section> => {
...
export const updateSection = async (
  id: string,
  updatedData: Partial<
    Omit<Section, "id" | "group" | "createdAt" | "updatedAt">
  >,
): Promise<Section> => {
