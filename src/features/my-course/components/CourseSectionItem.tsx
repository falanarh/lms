...
const getContentIcon = (type: string, isTabVariant: boolean = false) => {
  ...
  switch (type.toLowerCase()) {
    case "video":
      return (
        <div
          className={`${boxSize} rounded-md bg-blue-100 flex items-center justify-center flex-shrink-0`}
        >
    ...
    case "pdf":
      return (
        <div
          className={`${boxSize} rounded-md bg-red-100 flex items-center justify-center flex-shrink-0`}
        >
    ...
    case "link":
      return (
        <div
          className={`${boxSize} rounded-md bg-green-100 flex items-center justify-center flex-shrink-0`}
        >
    ...
    case "scorm":
      return (
        <div
          className={`${boxSize} rounded-md bg-purple-100 flex items-center justify-center flex-shrink-0`}
        >
    ...
    case "quiz":
      return (
        <div
          className={`${boxSize} rounded-md bg-orange-100 flex items-center justify-center flex-shrink-0`}
        >
    ...
    default:
      return (
        <div
          className={`${boxSize} rounded-md bg-gray-100 flex items-center justify-center flex-shrink-0`}
        >
...
export const CourseSectionItem = ({
  ...
  // Ensure contents are sorted by sequence
  const contents: Content[] | undefined =
    providedContents.length > 0
      ? [...providedContents].sort(
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-shrink-0">
                      {isPreviewMode ? (
                        <Lock
                          className={`${isTabVariant ? "w-4 h-5" : "w-3 h-3"} text-gray-500`}
                        />
                      ) : (
                    ...
                    <div className="flex-shrink-0">
                      {isCompleted ? (
                        <div
                          className={`${isTabVariant ? "w-6 h-6" : "w-4 h-4"} rounded-full bg-green-200 flex items-center justify-center`}
                        >
                          <Check
                            className={`${isTabVariant ? "w-4 h-4" : "w-3 h-3"} text-green-600`}
                            strokeWidth={3}
                          />
                        </div>
                      ) : isLocked ? (
                        <Lock
                          className={`${isTabVariant ? "w-5 h-5" : "w-4 h-4"} text-gray-500`}
                          strokeWidth={3}
                        />
                      ) : (
                        <div
                          className={`${isTabVariant ? "w-6 h-6" : "w-4 h-4"}`}
                        />
                      )}
                    </div>
                  </button>
